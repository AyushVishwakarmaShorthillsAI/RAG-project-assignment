...
def test_scrape_and_store_new_data(mocker, faiss_store, tmp_path):
    mocker.patch('app.data_processing.os.path.exists', return_value=False)
    scraper = MagicMock(spec=WikipediaScraper)
    scraper.scrape.side_effect = [["text1"*20], ["text2"*20]]

    faiss_store.embedding_model.encode.return_value = np.random.rand(2, 384)

    urls = ["url1", "url2"]
    index_file = tmp_path / "test_index.bin"
    texts_file = tmp_path / "test_texts.pkl"

    scrape_and_store(scraper, faiss_store, urls, str(index_file), str(texts_file))
...
def test_scrape_and_store_empty_urls(mocker, faiss_store, tmp_path):
    mocker.patch('app.data_processing.os.path.exists', return_value=False)
    scraper = MagicMock(spec=WikipediaScraper)
    scraper.scrape.return_value = []

    faiss_store.embedding_model.encode.return_value = np.array([])
    ...
    assert len(faiss_store.texts) == 0
    assert os.path.exists(index_file)
    assert os.path.exists(texts_file)
