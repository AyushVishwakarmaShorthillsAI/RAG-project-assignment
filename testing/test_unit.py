...
def test_scrape_success(setup_components, mocker):
    ...
    mock_get = mocker.patch('scraper.requests.get')
...
def test_ollama_generate_success(mocker):
    llm = OllamaLLM()
    mocker.patch('llm.requests.get', return_value=MagicMock(status_code=200))
...
def test_scrape_and_store_new_data(mocker):
    mocker.patch("data_processing.os.path.exists", return_value=False)
    scraper = MagicMock()
    vector_store = MagicMock()
...
# --- INTEGRATION TESTS ---
def test_full_rag_workflow(mocker):
    mocker.patch('main.FAISSVectorStore.load')
    mocker.patch('main.SentenceTransformer').return_value.encode.return_value = np.random.rand(1, 384)
    mocker.patch('main.OllamaLLM').return_value.generate.return_value = "Test answer"
    mocker.patch('main.st').return_value = MagicMock()
    
    main()
    
    # Verify UI components initialized
    assert 'last_question' in MagicMock().session_state
    assert 'answer' in MagicMock().session_state
